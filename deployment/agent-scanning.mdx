...
For older operating systems that do not support newer versions of PowerShell and/or cannot download the Shield Cyber Agent MSI file programmatically, the MSI can be downloaded manually and transferred over to the machine and installed manually.
...
<Note> Please update the file path location, as well as the SHIELD_AGENT_GROUP (Subscription ID) within the below script</Note>