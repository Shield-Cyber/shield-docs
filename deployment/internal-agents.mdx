...
description: "To perform individual scans on a host-by-host basis, Shield allows users to download and install agents across assets to assess the risks and vulnerabilities on each of those assets.  In order to install the agent on each machine, Shield provides a simple script to execute on each host (or across multiple hosts with a remote monitoring and management tool)."
---

### Deployment

Users can navigate directly to the [**Deploy Agents**](https://platform.shieldcyber.io/v2/dashboard?hub=deploy-agents) page to access a scripts that can be deployed on a host-by-host basis or deployed to multiple hosts via a preferred remote monitoring and management tool.

<Frame type="glass">
  <img src="/images/deploy-agents.png" alt="deploy" width="400"/>
</Frame>
...
Once the agents are deployed, users can navigate back to the [**Assets**](https://platform.shieldcyber.io/v2/assets) and refresh the page to view the newly deployed agents.

### Results

Once the agents can be seen in the platform within the [**Assets**](https://platform.shieldcyber.io/v2/assets) page, vulnerabilities will generally be available within five minutes throughout the platform.

### Additional Deployment Methods

#### Deploy Shield via Microsoft InTune

To deploy the Shield Internal Agent via InTune, users can leverage InTune's PowerShell scripting feature.
...
<Steps>
  ...
  <Step title="Navigate to the Shield platform & Agent deployment feature">
    Navigate to the [Shield platform](https://platform.shieldcyber.io/v2/dashboard?hub=deploy-agents), select the subscription you are looking to deploy from and copy the generated PowerShell script to a .ps1 file.
  </Step>
  ...
  <Step title="Set scope for devices configured within Intune, Review, & finish">
    Select the desired group and/or devices that you would like to deploy the Shield Internal Agent to and select **Next**.  Review all options and then click **Add** to your Intune PowerShell scripts.
    <Frame type="glass">
...
<Note> This deployment method should be utilized to understand how to deploy the Shield Internal Agent (via MSI) on multiple machines through Group Policy in Windows 2016 environments</Note>
...
Please follow the steps listed below to install the Shield Internal Agent with Group Policy: 
